    ...
    with client_list_lock:
        for client_info in client_list:
            ...
            if client_conn != sender_socket:
                try:
                    if message_content == "Exit" and sender_socket is not None: # Check sender_socket for server messages
                        full_message = f"{sender_username} has left the chat."
                    # For server announcements like "User has joined"
...
def handle_client(client_socket, client_address):
    ...
    # --- MODIFIED: Username acquisition and validation loop ---
    chosen_username = ""
    while True:
        try:
            client_socket.send("Please enter your username: ".encode()) # Send prompt
            username_attempt = client_socket.recv(1024).decode().strip() # Receive attempt
            ...
            if is_taken:
                ...
                client_socket.send(acceptance_msg.encode()) # Send acceptance
            ...
            except OSError: pass # Ignore if can't send
            finally:
                client_socket.close()
            return
    # --- END MODIFICATION ---
